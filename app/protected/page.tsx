...
const prompt = `Generate a funny, sarcastic roast for someone with the rank "${rank}" who has watched ${reelsCount} Instagram Reels. The rank system goes from best to worst: Sigma (0 reels) → Alpha (1-19) → Beta (20-49) → NPC (50-99) → Lost Soul (100-199) → Unemployed (200+). Lower ranks are worse, so roast accordingly. The roast should be witty and humorous but not mean-spirited. Keep it under 100 characters and include an emoji. Make it specific to their rank and reel count.`;
	...
	const currentUserGlobalPosition = globalLeaderboard.findIndex(user => user.isCurrentUser) + 1;
	const currentUserFriendsPosition = friendsLeaderboard.findIndex(user => user.isCurrentUser) + 1;
	const currentUserPosition = leaderboardTab === 'global' ? currentUserGlobalPosition || 'N/A' : currentUserFriendsPosition || 'N/A';
						...
						<div className="flex flex-col lg:flex-row items-center gap-6">
							...
							<div className="flex-1 text-center lg:text-left">
								...
								<p className="text-slate-300 text-lg mb-4 leading-relaxed">
									Install our Chrome extension to automatically track your Instagram Reels and compete with friends on the leaderboard of digital shame.
								</p>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
					<div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-slate-700/50 hover:border-emerald-500/50 transition-all duration-300 group">
						<div className="flex items-center justify-between mb-3">
							<span className="text-slate-400 text-sm font-medium">Total Reels Watched</span>
							<Target className="w-5 h-5 text-emerald-400 group-hover:scale-110 transition-transform" />
						</div>
						<div className="text-4xl font-bold text-white mb-1">{currentUser.reelsTotal}</div>
					...
					<div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-slate-700/50 hover:border-yellow-500/50 transition-all duration-300 group">
						<div className="flex items-center justify-between mb-3">
							<span className="text-slate-400 text-sm font-medium">Time Wasted (h)</span>
							<Clock className="w-5 h-5 text-yellow-400 group-hover:scale-110 transition-transform" />
						</div>
						<div className="text-4xl font-bold text-white mb-1">{(currentUser.timeWasted / 3600).toFixed(1)}</div>
						...
						<div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-slate-700/50">
							<div className="flex items-center justify-between mb-6">
								<div className="flex items-center gap-3">
									...
									<h2 className="text-2xl font-bold bg-gradient-to-r from-yellow-400 to-orange-400 bg-clip-text text-transparent">Leaderboard of Shame</h2>
								</div>
							...
							{leaderboardTab === 'friends' ? (
								<div className="text-center py-12">
									...
									<p className="text-slate-500 text-sm">Friends leaderboard feature is currently in development</p>
								</div>
							) : (
								<div className="space-y-3">
									{currentLeaderboard.slice(0, 5).map((user, index) => (
										...
										>
											...
											<div className="text-right">
												<div className="text-base font-bold text-white">{user.watchedReels}</div>
							...
							{leaderboardTab === 'global' && (
								<div className="mt-6 text-center p-4 bg-slate-900/50 rounded-xl border border-slate-700/50">
									<div className="text-sm text-slate-300">
										Your position: <span className="text-emerald-400 font-bold text-lg">#{currentUserPosition}</span>
										{globalLeaderboard.length > 0 && <span className="text-slate-500 ml-2">out of {globalLeaderboard.length} users</span>}
									</div>
								</div>
							)}
						</div>
					...
					<div className="space-y-6">
						...
						<div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-slate-700/50">
							...
							<div className="text-center mb-6">
								...
								<div className="text-sm text-slate-400 font-medium">Your Current Rank</div>
							</div>
						</div>
						...
						<div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-slate-700/50">
							...
							<div className="space-y-4">
								...
								<div className="flex justify-between items-center py-2">
									<span className="text-slate-400 font-medium">Time Wasted (h)</span>
									<span className="text-yellow-400 font-bold text-lg">
										{(currentUser.timeWasted / 3600).toFixed(1)}
									</span>
								</div>
							</div>
						</div>
						...
						<div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-slate-700/50">
							...
							<div className="text-center">
								<div className="text-4xl font-bold text-yellow-400 mb-2 drop-shadow-lg">#{currentUserPosition}</div>
								<div className="text-sm text-slate-400 mb-4 font-medium">{leaderboardTab === 'global' ? 'Global' : 'Friends'} Leaderboard</div>
								{leaderboardTab === 'global' && globalLeaderboard.length > 0 && currentUserGlobalPosition > 0 && (
									<div className="text-xs text-slate-500 bg-slate-900/50 px-3 py-2 rounded-lg border border-slate-700/50">
										You're ahead of {globalLeaderboard.length - currentUserGlobalPosition} other scrollers
									</div>
								)}
							</div>
						</div>
